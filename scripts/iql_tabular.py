...
class TabularQ(BaseLearningModel):
    """Independent tabular Q-learning for URB's single-step decisions."""

    def __init__(self, action_space_size, rng, bin_width=1.0,
                 eps_init=0.99, eps_decay=0.998, eps_min=0.0, lr=0.1):
        super().__init__()
        if not 0.0 < lr <= 1.0:
    ...
    def act(self, state):
        ...
        if self.rng.random() < self.epsilon:
            ...
            best_value = np.max(q_values[valid_actions])
            best_actions = valid_actions[q_values[valid_actions] == best_value]
    ...
    def learn(self):
        ...
        td_error = self.last_reward - q_values[self.last_action]
        q_values[self.last_action] += self.lr * td_error
        self.loss.append(float(td_error ** 2))
        self.decay_epsilon()
        del self.last_state, self.last_action, self.last_reward

    def decay_epsilon(self):
        self.epsilon = max(self.eps_min, self.epsilon * self.eps_decay)


# Main script to run the tabular IQL experiment
if __name__ == "__main__":
    ...
    for idx in range(len(env.machine_agents)):
        env.machine_agents[idx].model = TabularQ(
    ...
    for episode in range(training_eps):
        ...
        if should_save_model_snapshot(completed_episode, training_eps, save_model_every):
            with open(
                model_snapshot_path(records_folder, completed_episode, "pkl"),
                "wb",
            ) as f:
                pickle.dump(
                    {
