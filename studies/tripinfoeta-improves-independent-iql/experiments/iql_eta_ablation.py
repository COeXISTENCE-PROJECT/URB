...
class DQN(BaseLearningModel):
    ...
    def act(self, state):
        if self.rng.random() < self.epsilon:
            action = self.rng.choice(self.action_space_size)
        else:
    ...
    def learn(self):
        ...
        for _ in range(self.num_epochs):
            batch = self.replay_rng.sample(self.memory, self.batch_size)
...
def study_observation(observation, agent, freeflows, eta_mode, number_of_paths):
    observation = observation.copy()
    if eta_mode == "static":
        observation[:number_of_paths] = freeflows[(agent.origin, agent.destination)]
    return observation


def daily_metrics(env, phase, episode):
    rows = env.last_episode_travel_times
    cav_times = [row["travel_time"] for row in rows if row["kind"] == "AV"]
    human_times = [row["travel_time"] for row in rows if row["kind"] == "Human"]
    return {
        "phase": phase,
        "episode": episode,
        "t_CAV": np.mean(cav_times),
        "t_HDV": np.mean(human_times),
        "t_system": np.mean([row["travel_time"] for row in rows]),
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eta-mode', choices=['dynamic', 'static'], required=True)
    parser.add_argument('--paths-from', type=str)
    ...
    # Parameter setting
    params = dict()
    alg_params = json.load(open(os.path.join(script_dir, "configs", f"{alg_config}.json")))
    env_params = json.load(open(os.path.join(script_dir, "configs", f"{env_config}.json")))
    task_params = json.load(open(os.path.join(script_dir, "configs", f"{task_config}.json")))
    ...
    custom_network_folder = os.path.join(repo_root, "networks", network)
    ...
    if eta_mode == "static":
        if args.paths_from is None:
            raise ValueError("Static ETA runs require --paths-from")
        for filename in ["routes.csv", "route.rou.xml"]:
            shutil.copy2(os.path.join(args.paths_from, filename), records_folder)
    ...
    dump_config["eta_mode"] = eta_mode
    dump_config["test_save_every"] = 1
    ...
    freeflows = env.observation_obj.freeflows
    ...
    for idx in range(len(env.machine_agents)):
        env.machine_agents[idx].model = DQN(obs_size, env.machine_agents[idx].action_space_size, 
                                            ...
                                            replay_rng=random.Random(torch_seed + idx),
        ...
        for agent_id in env.agent_iter():
            ...
            if termination or truncation:
                ...
                observation = study_observation(
                    observation, agent, freeflows, eta_mode, number_of_paths
                )
                action = agent.model.act(observation)
                
            env.step(action)
            
        if episode % plot_every == 0:
            env.plot_results()
        day_metrics.append(daily_metrics(env, "training", episode + 1))
        pbar.update()
    ...
    for episode in range(test_eps):
        ...
        for agent_id in env.agent_iter():
            ...
            if termination or truncation:
                ...
                action = agent.model.act(observation)
            env.step(action)
        day_metrics.append(daily_metrics(env, "testing", episode + 1))
        pbar.update()
    ...
    torch.save(
        {str(agent.id): agent.model.q_network.state_dict() for agent in env.machine_agents},
        os.path.join(models_folder, "policies.pt"),
    )
    metrics_folder = os.path.join(records_folder, "metrics")
    os.makedirs(metrics_folder, exist_ok=True)
    pd.DataFrame(day_metrics).to_csv(
        os.path.join(metrics_folder, "DailyMetrics.csv"), index=False
    )

    env.stop_simulation()
    clear_SUMO_files(os.path.join(records_folder, "SUMO_output"), os.path.join(records_folder, "episodes"), remove_additional_files=True)
    if not args.skip_metrics:
        run_metrics_analysis(exp_id, results_folder=results_folder)
